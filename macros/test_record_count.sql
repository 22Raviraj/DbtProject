...
{% set source_where = where_condition.base if (where_condition.base is defined and where_condition.base|string | length > 0) else '' %}
{% set target_where = where_condition.target if (where_condition.target is defined and where_condition.target|string | length > 0) else '' %}
{% set source_multiplier = count_multiplier.base if (count_multiplier.base is defined) else 1 %}
{% set target_multiplier = count_multiplier.target if (count_multiplier.target is defined) else 1 %}
